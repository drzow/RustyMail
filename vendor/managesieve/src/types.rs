...
impl TryFrom<(&str, Option<&str>)> for Capability {
    ...
    fn try_from(s: (&str, Option<&str>)) -> Result<Self, Self::Error> {
        ...
        Ok(match cap {
            ...
            // Upstream 0.1.1 mis-mapped LANGUAGE -> Owner; fixed locally.
            "LANGUAGE" => Capability::Language(unwrap_rest()?),
...
///
/// Returns `(remaining_input, script_body, response)`. `script_body` is
/// `None` when the server replied NO/BYE without a script (e.g.
/// NONEXISTENT) — callers should inspect `response.tag` / `response.code`
/// in that case.
///
/// Local fix: upstream 0.1.1 collapsed the NO/BYE branch into
/// `InvalidResponse`, hiding the typed response code from callers.
pub fn response_getscript(
    input: &str,
) -> Result<(&str, Option<String>, Response), Error> {
    match p::response_getscript(input) {
        Ok((left, (script, resp))) => Ok((left, script, resp)),
        Err(nom::Err::Incomplete(_)) => Err(Error::IncompleteResponse),
        _ => Err(Error::InvalidResponse),
    }
}
